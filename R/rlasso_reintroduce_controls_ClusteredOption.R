#'  rlasso_reintroduce_controls_ClusteredOption.R    Function for rlasso estimation - with unpenalized coefficients and with a clustering option
#'  
...
rlasso_reintroduce_controls<-function (x, y,index_x_no_selection,clusters, post = TRUE, intercept = TRUE, model = TRUE, 
                                                                                           ...
                                                                                           tol = 10^-5, threshold = NULL), ...) 
{
  ...
  while (mm <= control$numIter) {
    ...
    selected<-sum(ind1)
    ## =========== (Addition) Force some variables into the Post Lasso Model/Selection set ===== ##
    ind1[index_x_no_selection]<- T 
